        ...
        with Tx.cta():
            ...
            with Tx.thread():
                ...
                while self.token_idx[0] < self.num_tokens:
                    ...
                    thread_read_ptr_offset = Tx.meta_var(thread_row * self.ELTS_PER_ROW + first_elt_read_by_thread)

                    if thread_row < self.num_tokens:
                        # copy global to reg
                        for ii in Tx.unroll(self.LDG_PER_THREAD):
                            thread_read_ptr_offset1 = Tx.meta_var(thread_read_ptr_offset + ii * self.THREADS_PER_ROW * self.ELTS_PER_LDG)
                    ...
                    self.row_sum[0] = 0
                    for ii in Tx.unroll(self.VPT):
                        self.row_chunk[ii] = Tx.exp(Tx.Cast("float32", self.row_chunk_temp[ii]) - Tx.Cast("float32", self.thread_max[0]))
                    ...
                    for k_idx in Tx.serial(self.topk):
                        ...
                        if thread_group_idx == 0 and thread_row < self.num_tokens:
                            ...
                            should_process_row = Tx.meta_var(Tx.bool((self.expert[0] >= start_expert) and (self.expert[0] < end_expert)))
                            ...
                            topk_indices[idx0, idx1] = Tx.if_then_else(should_process_row, self.expert[0] - start_expert, self.num_experts)
                            self.row_sum_for_renormalize[0] = self.row_sum_for_renormalize[0] + self.max_val[0]

                        # clear value in thread
                        if k_idx + 1 < self.topk:
                            ldg_group_for_expert = Tx.meta_var(self.expert[0] // self.COLS_PER_GROUP_LDG)
                            thread_to_clear_in_group = Tx.meta_var((self.expert[0] // self.ELTS_PER_LDG) % self.THREADS_PER_ROW)
                            if thread_group_idx == thread_to_clear_in_group:
                                offset_for_expert = Tx.meta_var(self.expert[0] % self.ELTS_PER_LDG)
                                idx = Tx.meta_var(ldg_group_for_expert * self.ELTS_PER_LDG + offset_for_expert)
                    ...
                    if renormalize and thread_group_idx == 0 and thread_row < self.num_tokens:
                        row_sum_for_renormalize_inv = Tx.meta_var(1.0 / self.row_sum_for_renormalize[0])
