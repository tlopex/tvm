...
"""
    ...
    def _alloc_prelogue(self):
        ...
        self.thr_local_kv_offset = Tx.alloc_local([self.num_mma_kv * self.kv_thr_layout_col // 2 // self.num_warps_q], "int64", name="thr_local_kv_offset")
    ...
    def _init_state(
        ...
        with Tx.cta():
            ...
            self.qo_upperbound = Tx.min(self.q_len[0], ceildiv(self.qo_packed_idx_base[0] + self.cta_tile_q, self.gqa_group_size)) 
            ...
            def prefetch_offset(packed_block_iter_base_in):
                with Tx.thread():
                    ...
                    for i in Tx.unroll(self.num_mma_kv * 4 // self.num_warps_q):
                        ...
                        page_iter = int_var(name="page_iter", val=Tx.floordiv(packed_block_iter[0], self.page_size))
            ...
            with Tx.thread():
                ...
                def handle_single_task(has_prefetched):                
                        ...
                        if self.get_warp_idx_kv(self.tid) == 0:
                            ...
                            for mma_q in Tx.unroll(self.num_mma_q):
                                for j in Tx.unroll(4):
                                    with Tx.thread():
                                        qo_packed_id = Tx.meta_var(self.qo_packed_idx_base[0] + lane_id // 8 + mma_q * 16 + j * 4)
                                        q = int_var(name="q", val=Tx.floordiv(qo_packed_id, self.gqa_group_size))
                                        r = int_var(name="r", val=Tx.floormod(qo_packed_id, self.gqa_group_size))
                                        for mma_do in Tx.unroll(self.num_mma_d_qk // 4):
                                            Tx.ptx.cp_async(self.q_smem.ptr_to([q_smem_offset_w[0] * upcast_size("float16")]),
                                                            # TODO: optimize the addr computation here
                    ...
                    def compute_qk():
                        with Tx.thread():
                            ...
                            for mma_d in Tx.unroll(self.num_mma_d_qk):
                                for mma_q in Tx.unroll(self.num_mma_q):
                                    Tx.ptx.ldmatrix(False, 4, ".b16", a_frag.ptr_to([mma_q, 0]), self.q_smem.ptr_to([self.q_smem_offset_r[0] * upcast_size("float16")]))
                                ...
                                for mma_kv in Tx.unroll(self.num_mma_kv):
                                    Tx.ptx.ldmatrix(False, 4, ".b16", b_frag.ptr_to([0]), self.k_smem.ptr_to([self.k_smem_offset_r[0] * upcast_size("float16")]))
                        ...
                        with Tx.thread():
                            ...
                            for mma_q in Tx.unroll(self.num_mma_q):
                                for j in Tx.unroll(2):
                                    ...
                                    for mma_kv in Tx.unroll(self.num_mma_kv):
                                        m_local = float_var(name="m_local", val=Tx.max(Tx.max(self.s_frag[mma_q, mma_kv, j * 2 + 0], self.s_frag[mma_q, mma_kv, j * 2 + 1]),
                                                                    Tx.max(self.s_frag[mma_q, mma_kv, j * 2 + 4], self.s_frag[mma_q, mma_kv, j * 2 + 5])))
                                        self.m[mma_q, j] = Tx.max(self.m[mma_q, j], m_local[0])
                                    self.m[mma_q, j] = Tx.max(self.m[mma_q, j], Tx.tvm_warp_shuffle_xor(WARP_MASK, self.m[mma_q, j], 0x2, 32, 32))
                                    self.m[mma_q, j] = Tx.max(self.m[mma_q, j], Tx.tvm_warp_shuffle_xor(WARP_MASK, self.m[mma_q, j], 0x1, 32, 32))
                        ...
                        with Tx.thread():
                            ...
                            for mma_kv in Tx.unroll(self.num_mma_kv):
                                for mma_d in Tx.unroll(self.num_mma_d_vo):
                                    with Tx.thread():
                                        b_frag = Tx.alloc_local([4], "uint32", name="b_frag")
                                        Tx.ptx.ldmatrix(True, 4, ".b16", b_frag.ptr_to([0]), self.v_smem.ptr_to([self.v_smem_offset_r[0] * upcast_size("float16")]))
                                        for mma_q in Tx.unroll(self.num_mma_q):
                    ...
                    def threadblock_sync_mdo_states():
                        ...
                        for mma_q in Tx.unroll(self.num_mma_q):
                            with Tx.thread():
                                o_scale = Tx.alloc_local([2, self.num_warps_kv], "float32", name="o_scale")
                                for j in Tx.unroll(2):
                                    with Tx.thread():
                    ...
                    def normalize_d():
                        with Tx.thread():
                            ...
                            for mma_q in Tx.unroll(self.num_mma_q):
                                for j in Tx.unroll(2):
                                    d_rcp[mma_q, j] = Tx.if_then_else(self.m[mma_q, j] != -self.inf, ptx_rcp(self.d[mma_q, j]), 0.0)
                            for mma_q in Tx.unroll(self.num_mma_q):
                                for mma_d in Tx.unroll(self.num_mma_d_vo):
                                    for reg_id in Tx.unroll(8):
                                ...
                                for mma_q in Tx.unroll(self.num_mma_q):
                                    for j in Tx.unroll(4):
                                        with Tx.thread():
                                            ...
                                            q = int_var(name="q", val=Tx.floordiv(o_packed_idx[0], self.gqa_group_size))
                                            r = int_var(name="r", val=Tx.floormod(o_packed_idx[0], self.gqa_group_size))
                                ...
                                for mma_q in Tx.unroll(self.num_mma_q):
                                    for j in Tx.unroll(2):
                                        with Tx.thread():
                                            ...
                                            q = int_var(name="q", val=Tx.floordiv(packed_qo_idx[0], self.gqa_group_size))
                                            r = int_var(name="r", val=Tx.floormod(packed_qo_idx[0], self.gqa_group_size))
                                            if q[0] < self.qo_upperbound[0]:                                                    
                                                partial_lse_buf_offset = Tx.meta_var((self.o_indptr[0] + (packed_qo_idx[0] - self.packed_qo_start[0]) * self.num_kv_chunks[0] + self.kv_chunk_idx[0]) * self.kv_heads + self.kv_head_idx[0])
                                                partial_lse_tvm[partial_lse_buf_offset] = ptx_log2(self.d[mma_q, j]) + Tx.cast(self.m[mma_q, j], "float32")
