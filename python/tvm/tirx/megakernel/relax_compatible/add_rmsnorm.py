...
class FuseAddRMSNormTile(AddRMSNormTile):
    ...
    def get_func(batch_size, rms_norm_eps, hidden_size):
        ...
        def add_rmsnorm_func(input_ptr: Tx.handle, residual_ptr: Tx.handle, weight_ptr: Tx.handle, output_ptr: Tx.handle, out_residual_ptr: Tx.handle, m_idx: Tx.int32, n_idx: Tx.int32, k_idx: Tx.int32):
            Tx.func_attr({"megakernel.device_func": "add_rmsnorm"})
            seq_len = Tx.int32()
            input = Tx.match_buffer(input_ptr, (seq_len, hidden_size), "float16", scope="global")
            residual = Tx.match_buffer(residual_ptr, (seq_len, hidden_size), "float16", scope="global")
...
class FuseRMSNormTile(RMSNormTile):
    ...
    def get_func(batch_size, rms_norm_eps, hidden_size):
        ...
        @Tx.prim_func(tirx=True, private=True)
        def rmsnorm_func(input_ptr: Tx.handle, weight_ptr: Tx.handle, output_ptr: Tx.handle, m_idx: Tx.int32, n_idx: Tx.int32, k_idx: Tx.int32):
