...
def codegen_ptx_mma(
    ...
    shape = str(shape)[1:-1]
    a_layout = str(a_layout)[1:-1]
    b_layout = str(b_layout)[1:-1]
    d_type = str(d_type)[1:-1]
    a_type = str(a_type)[1:-1]
    b_type = str(b_type)[1:-1]
    c_type = str(c_type)[1:-1]
    saturate = bool(saturate)
    if bit_op is not None:
        bit_op = str(bit_op)[1:-1]
