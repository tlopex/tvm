...
class StmtMutator(StmtFunctor):
    ...
    def visit_for_(self, op):
        ...
        return tvm.tir.For(
            op.loop_var, min_val, extent, op.kind, body, 
    ...
    def visit_allocate_(self, op):
        ...
        return tvm.tir.Allocate(
            op.buffer_var, op.dtype, extents, condition, body, 
    ...
    def visit_allocate_const_(self, op):
        ...
        return tvm.tir.AllocateConst(
            op.buffer_var, op.dtype, extents, data_or_idx, body, 
            op.annotations, op.span
        )
    ...
    def visit_block_(self, op):
        ...
        for iv in op.iter_vars:
            ...
            if new_min is not old_dom.min or new_extent is not old_dom.extent:
                ...
                iter_vars.append(
                    tvm.tir.IterVar(new_dom, iv.var, iv.iter_type, iv.thread_tag)
                )
            else:
                iter_vars.append(iv)
        ...
        if (not iter_vars_changed and 
            not reads_changed and 
            not writes_changed and 
            not match_buffers_changed and 
            (init is op.init or (init is None and op.init is None)) and 
            body is op.body):
            return op
