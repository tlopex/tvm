...
def get_buffer_size(buffer: Buffer) -> int:
    ...
    if buffer.scope() == "trn.sbuf":
        ...
        raise ValueError(f"Unsupported buffer scope: {buffer} ({buffer.scope()})")
...
class BufferReplacer(StmtExprMutator):
    ...
    def visit_op_call_(self, op):
        ...
        new_workspace = {key: self.buffer_map[value] if value in self.buffer_map else value for key, value in op.workspace.items()}
        return OpCall(*op.args, op=op.op, workspace=new_workspace, schedule_config=op.schedule_config)
...
class AllocMutator(BufferReplacer):
    ...
    def visit_block_(self, op: Block):
        ...
        for buffer in op.alloc_buffers:
            ...
            changed = True
            self.alloc_offset += get_buffer_size(buffer)
