...
from .common import Tile, KernelConfig, F16_BYTES, ceildiv, find_power_of_two, exp2
                        ...
                        for kb in T.unroll(self.loop_inner):
                            ...
                            for kz in T.unroll(self.bdz):
                                if self.tile_per_bdx * self.bdy * kz < self.chunk_size[0]:
                                    ...
                                    self.d[kb, 0] = (self.d[kb, 1] * exp2(self.m[kb, 1] - self.m[kb, 0]) 
                                                        + self.d_tmp[kb, 0] * exp2(self.m_tmp[kb, 0] - self.m[kb, 0]))
                                    for kv in T.unroll(self.vec_size):
                                        self.o[kb, kv] = (self.o[kb, kv] * exp2(self.m[kb, 1] - self.m[kb, 0]) 
                                                            + self.o_tmp[kb, kv] * exp2(self.m_tmp[kb, 0] - self.m[kb, 0]))
                            # normalize
                            for kv in T.unroll(self.vec_size):
