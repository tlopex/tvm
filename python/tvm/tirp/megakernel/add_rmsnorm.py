        ...
        with T.cta():
            ...
            with T.thread():
                ...
                for ki in T.serial(ceildiv(self.hidden_size, self.vec_size * KernelConfig.NUM_THREADS * self.loop_inner)):                    
                    ...
                    for kl in T.unroll(self.loop_inner):
                        if st < self.hidden_size - kl * self.vec_size:
                            Tp.copy(self.input_vec_f32[kl, :], input[m_idx, st + kl * self.vec_size : st + kl * self.vec_size + self.vec_size])
                    for kl in T.unroll(self.loop_inner):
                        for kv in T.unroll(self.vec_size):
                            self.sum_sq[kl, 0] += self.input_vec_f32[kl, kv] * self.input_vec_f32[kl, kv]
                    for kl in T.unroll(self.loop_inner):
                        if st < self.hidden_size - kl * self.vec_size:
                            Tp.copy(self.x_smem[st + kl * self.vec_size : st + kl * self.vec_size + self.vec_size], self.input_vec_f32[kl, :])
                ...
                for ki in T.serial(ceildiv(self.hidden_size, self.vec_size * KernelConfig.NUM_THREADS)):                    
                    ...
                    for kl in T.unroll(self.loop_inner):
                        if st < self.hidden_size - kl * self.vec_size:
                            Tp.copy(self.weight_vec[kl, :], weight[st + kl * self.vec_size : st + kl * self.vec_size + self.vec_size])
                            Tp.copy(self.input_vec_f32[kl, :], self.x_smem[st + kl * self.vec_size : st + kl * self.vec_size + self.vec_size])
                    for kl in T.unroll(self.loop_inner):
                        Tp.cast(self.weight_vec_f32[kl, :], self.weight_vec[kl, :])
                    for kl in T.unroll(self.loop_inner):
                        for kv in T.unroll(self.vec_size):
                    ...
                    for kl in T.unroll(self.loop_inner):
                        if st < self.hidden_size - kl * self.vec_size:
                            Tp.copy(output[m_idx, st + kl * self.vec_size : st + kl * self.vec_size + self.vec_size], self.input_vec[kl, :])
