...
def binary_reduce_trn(
    ...
    intermediate_shape, intermediate_layout, reduction_b_extent = generate_intermediate_buffer(
        reduce_output, binary_output, reduce_axes, inst_size, analyzer
    )
    ...
    f_gen_reduce_dst_idx = f_gen_idx_mapped(
        reduce_output, f_gen_axes, get_reduction_dim_map(binary_output, reduce_output, reduce_axes)
    )
    p_size = binary_output.buffer.layout.partition_size
    b_extent = (
        reduce(operator.mul, [r.extent for r in binary_output.region], 1) // p_size // inst_size
    )
    src1, src2, dst1, dst2 = (
...
def unary_reduce_trn(
    ...
    if bias is not None:
        ...
        inst_size, f_gen_axes = try_find_inst_unary(
            unary_output, unary_input, analyzer, allowed_f_dim_dst=reduce_axes
        )
    assert analyzer.can_prove(inst_size > 1), "Instruction size must be greater than 1"
    intermediate_shape, intermediate_layout, reduction_b_extent = generate_intermediate_buffer(
        reduce_output, unary_output, reduce_axes, inst_size, analyzer
    )
    ...
    f_gen_reduce_dst_idx = f_gen_idx_mapped(
        reduce_output, f_gen_axes, get_reduction_dim_map(unary_output, reduce_output, reduce_axes)
    )
    ...
    b_extent = (
        reduce(operator.mul, [r.extent for r in unary_output.region], 1) // p_size // inst_size
    )
...
def binary_chain_trn(
    ...
    inst_size, f_gen_axes, inst_types, _reverse, broadcast_dims = try_find_inst_nary(
        output, srcs, analyzer, allow_first_op_tensortensor=False
    )
    ...
    b_extent = reduce(operator.mul, [r.extent for r in output.region], 1) // p_size // inst_size
