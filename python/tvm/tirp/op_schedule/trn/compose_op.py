...
from .reduction import generate_intermediate_buffer
from ..common import MapOpType, ReduceOpType
from ..registry import f_op_scheduler


vector_ops = {
    Op.get("tirp.add"): (MapOpType.ADD, "add"),
...
reduce_ops_after_vector = {
    Op.get("tirp.sum"): (ReduceOpType.SUM, "sum"),
...
def compose_vector_reduce(
    ...
    assert not is_tensor_tensor, "TensorTensor is not supported for vector reduce"
    ...
    opcode, reduce_opcode = vector_ops[op_call_1.op][1], reduce_ops_after_vector[op_call_2.op][1]
                ...
                with T.attr(0, "tensorized_nki_instruction", 1):
                    for p_loop, f_loop in T.grid(p_size, inst_size):
                        ...
                        if CONST is None:
                            ...
                            T.nki_tensorscalar_reduce(dst2[*reduce_dst_idx], dst1[*vec_dst_idx], src1[*src_1_indices], src2[*src_2_indices], opcode, reduce_opcode, reorder)
                        else:
                            T.nki_tensorscalar_reduce(dst2[*reduce_dst_idx], dst1[*vec_dst_idx], src1[*src_1_indices], CONST, opcode, reduce_opcode, reorder)
        # fmt: on
        return impl
    else:
                        ...
                        with T.attr(0, "tensorized_nki_instruction", 1):
                            for p_loop, f_loop in T.grid(p_size, inst_size):
                                ...
                                if CONST is None:
                                    ...
                                    T.nki_tensorscalar_reduce(intermediate_buffer[p_loop, reduction_b_loop], dst1[*vec_dst_idx], src1[*src_1_indices], src2[*src_2_indices], opcode, reduce_opcode, reorder)
                                else:
                                    T.nki_tensorscalar_reduce(intermediate_buffer[p_loop, reduction_b_loop], dst1[*vec_dst_idx], src1[*src_1_indices], CONST, opcode, reduce_opcode, reorder)
                    with T.attr(0, "tensorized_nki_instruction", 1):
                        for p_loop, f_loop in T.grid(p_size, reduction_b_extent):
                            ...
                            T.nki_tensorreduce(dst2[*dst_2_indices], intermediate_buffer[p_loop, f_loop], reduce_opcode, -1)
        # fmt: on
        return impl
