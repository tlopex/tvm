...
def select_trn(op: OpCall, sctx: ScheduleContext) -> Optional[PrimFunc]:
    ...
    inst_size, inst_stride, inst_data_iters = find_max_inst_size_unary(dst, true_value, analyzer)
    ...
    f_gen_axes = generate_axes_in_region(dst, inst_stride, inst_data_iters, analyzer)
    ...
    b_extent = reduce(operator.mul, dst_extent, 1) // inst_size // p_size
