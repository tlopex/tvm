...
inline Tensor meta_schedule_layout_transform(
  ...
  ffi::Array<PrimExpr> post_transform_shape = index_map->MapShape(src->shape, analyzer.get());
  return compute(
      post_transform_shape,
      [src, inv = index_map.Inverse(iter_domain, analyzer.get()),
       &analyzer](const ffi::Array<Var>& indices) -> PrimExpr {
        return src(
            inv->MapIndices(ffi::Array<PrimExpr>{indices.begin(), indices.end()}, analyzer.get()));
      },
      name, tag);
}
