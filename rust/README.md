...
The prototype deliberately exposes only opaque IR handles: TVM owns the
complete C++ object layout, field access uses runtime reflection, construction
uses registered TVM functions, and recursion/rebuilding uses `tvm-ffi`'s
language-agnostic structural protocol.
