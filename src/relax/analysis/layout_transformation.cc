...
static bool IsBijectiveAffine(const IndexMap& m, const ffi::Array<Range>& ranges) {
  ...
  auto iter_map_result = DetectIterMap(m->final_indices, input_iters, /* predicate = */ 1,
                                       /*check_level=*/arith::IterMapLevel::Bijective, analyzer.get(),
...
static bool AreIdenticalTransforms(const IndexMap& t0, const IndexMap& t1) {
  ...
  arith::Analyzer analyzer;
  auto t0_output = t0->MapIndices(t1_initial_indices, analyzer.get());
 ...
 private:
  ...
  SpatialLayout DetectBufferAccessIterMap(ffi::Array<PrimExpr> indices) {
    auto result = arith::DetectIterMap(
        ...
        /*predicate*/ 1, /*check_level*/ arith::IterMapLevel::NoCheck, arith_analyzer_.get());
