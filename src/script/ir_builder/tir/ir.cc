...
ffi::Array<tvm::tir::Var> CtaId(ffi::Array<PrimExpr> extents, ffi::String parent,
  ...
  if (preferred.defined()) {
    ...
    const_cast<tvm::tir::ExecScopeNode*>(exec_scope.value().as<tvm::tir::ExecScopeNode>())
        ->scope_id_def.push_back(tvm::tir::ScopeIdDef(
            scope_ids, extents, tvm::tir::ScopePair(parent, "cta"), preferred));
    return scope_ids;
  }
  return ScopeId(extents, parent, "T.cta_id", "cta");
}

ffi::Array<tvm::tir::Var> WarpgroupId(ffi::Array<PrimExpr> extents, ffi::String parent) {
...
void BlockAttrs(ffi::Map<ffi::String, Any> attrs) {
  ...
  if (sblock_frame.defined()) {
    ...
    TVM_FFI_CHECK(!(func_frame.defined() && func_frame.value()->is_tirx), ValueError)
...
ffi::Variant<Buffer, AllocBufferFrame> SBlockAllocBuffer(
  ...
  TVM_FFI_ICHECK(opt_func_frame.has_value())
      << "ValueError: PrimFunc frame not find. Please ensure "
  ...
  if (ffi::Optional<SBlockFrame> block_frame = builder->GetLastFrame<SBlockFrame>()) {
    ...
    TVM_FFI_THROW(InternalError)
        << "ValueError: Block frame or PrimFunc frame not found. Please ensure "
