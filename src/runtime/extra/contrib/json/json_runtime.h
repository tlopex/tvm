 ...
 protected:
  ...
  void SetInputOutputBuffers(const ffi::PackedArgs& args) {
    ...
    for (int i = 0; i < args.size(); ++i) {
      ...
      } else if (args[i].as<ffi::Shape>()) {
        // Shape arguments bind symbolic dimensions in Relax, but do not correspond to data
        // entries in the serialized graph.
        continue;
      } else {
        TVM_FFI_THROW(TypeError) << "JSON runtime expects Tensor, DLTensor*, or Shape arguments, "
                                 << "but received " << args[i].GetTypeKey();
      }
