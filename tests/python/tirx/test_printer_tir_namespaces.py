...
def test_printer_ptx_more():
    ...
    _assert_print(tir.op.ptx_map_shared_rank(r, 2), "r = T.handle()\nT.ptx.map_shared_rank(r, 2)")
    ...
    _assert_print(
        tir.op.ptx_tcgen05_alloc(s, 64, 1), "s = T.handle()\nT.ptx.tcgen05.alloc(s, 64, 1)"
    )
    _assert_print(
        tir.op.ptx_tcgen05_dealloc(s, 64, 1), "s = T.handle()\nT.ptx.tcgen05.dealloc(s, 64, 1)"
    )
    d = tir.Var("d", "handle")
    a = tir.Var("a", "handle")
    b = tir.Var("b", "handle")
    ...
    _assert_print(tir.op.ptx_tcgen05_shift(a, 1), "a = T.handle()\nT.ptx.tcgen05.shift(a, 1)")
    _assert_print(
    ...
    _assert_print(
        tir.op.ptx_tcgen05_relinquish_alloc_permit(1), "T.ptx.tcgen05.relinquish_alloc_permit(1)"
    )
...
def test_printer_ptx_mbarrier():
    ...
    _assert_print(
        tir.op.ptx_mbarrier_init(bar, 32), "bar = T.handle()\nT.ptx.mbarrier.init(bar, 32)"
    )
    _assert_print(tir.op.ptx_mbarrier_arrive(bar), "bar = T.handle()\nT.ptx.mbarrier.arrive(bar)")
    _assert_print(
        tir.op.ptx_mbarrier_arrive_expect_tx(bar, 128),
        "bar = T.handle()\nT.ptx.mbarrier.arrive.expect_tx(bar, 128)",
    )
    _assert_print(
        tir.op.ptx_mbarrier_try_wait(bar, 1), "bar = T.handle()\nT.ptx.mbarrier.try_wait(bar, 1)"
    )
    _assert_print(tir.op.cuda_cluster_sync(), "T.cuda.cluster_sync()")
...
def test_printer_cuda_more():
    ...
    _assert_print(tir.op.cuda_cta_sync(), "T.cuda.cta_sync()")
    ...
    _assert_print(
        tir.op.cuda_atomic_add(p, tir.IntImm("int32", 1)), "p = T.handle()\nT.cuda.atomic_add(p, 1)"
    )
    _assert_print(tir.op.cuda_atomic_cas(p, 1, 2), "p = T.handle()\nT.cuda.atomic_cas(p, 1, 2)")
    _assert_print(tir.op.cuda_ldg(p, "float32"), 'p = T.handle()\nT.cuda.ldg(p, "float32")')
    _assert_print(tir.op.cuda_func_call("f", 1, source_code=""), 'T.cuda.func_call("f", 1, "")')
...
def test_printer_nvshmem_more():
    ...
    _assert_print(
        tir.op.nvshmem_getmem_nbi(p, p, 16, 0), "p = T.handle()\nT.nvshmem.getmem_nbi(p, p, 16, 0)"
    )
    _assert_print(
    ...
    _assert_print(
        tir.op.nvshmem_putmem_nbi(p, p, 16, 0), "p = T.handle()\nT.nvshmem.putmem_nbi(p, p, 16, 0)"
    )
    _assert_print(
