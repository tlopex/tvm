...
class MegaKernel:
    ...
    def _qwen3_layer_inner(self, bb: rx.BlockBuilder, max_batch_size, blk_m, profile_on):
        ...
        residual = rx.Var(
            "residual", rx.TensorStructInfo([batch_size, self.HIDDEN_SIZE], "float32")
        )
        packed_info = rx.Var(
            "packed_info",
            rx.TupleStructInfo(
        ...
        packed_weights = rx.Var(
            ...
            rx.TupleStructInfo(
                [
                    ...
                    rx.TensorStructInfo([self.HEAD_DIM], "float16"),
                    rx.TensorStructInfo(
                        [self.HIDDEN_SIZE, self.NUM_ATTENTION_HEADS * self.HEAD_DIM], "float16"
                    ),
        ...
        with bb.function(
            f"megakernel_blkm{blk_m}", [x, residual, packed_info, packed_weights, workspace]
        ):
            # with bb.dataflow():
            # unpack info
            ...
            kv_data_ = bb.match_cast(
                kv_data_,
                rx.TensorStructInfo(
                    [max_page_num, 2, self.NUM_KEY_VALUE_HEADS, self.PAGE_SIZE, self.HEAD_DIM],
                    "float16",
                ),
            )
            ...
            (
                reduce_rms_rope_q_func,
                reduce_rms_rope_q_num_tiles,
                reduce_rms_rope_q_size,
            ) = FuseSplitKReduceRMSnormRopeQTile.get_func(
                batch_size,
                self.RMS_NORM_EPS,
            ...
            (
                reduce_rms_rope_append_k_func,
                reduce_rms_rope_append_k_num_tiles,
                _,
            ) = FuseSplitKReduceRMSnormRopeAppendKTile.get_func(
                batch_size,
                self.RMS_NORM_EPS,
                self.NUM_ATTENTION_HEADS,
                self.NUM_KEY_VALUE_HEADS,
            ...
            (
                reduce_append_v_func,
                reduce_append_v_num_tiles,
                _,
            ) = FuseSplitKReduceAppendVTile.get_func(
                batch_size,
                self.NUM_ATTENTION_HEADS,
                self.NUM_KEY_VALUE_HEADS,
                self.HEAD_DIM,
            ...
            (
                gate_up_silu_func,
                gate_up_silu_num_tiles,
                gate_up_silu_size,
            ) = FuseGateUpSiluTile.get_func(
            ...
            reduce_rms_rope_append_k_gv = bb.add_func(
                reduce_rms_rope_append_k_func, "reduce_rms_rope_append_k"
            )
            ...
            etensor_down_proj = bb.emit(
                R.alloc_event_tensor(
                    ...
                    f_init=lambda i: f_init_unmatched_dim(
                        self.INTERMEDIATE_SIZE, gate_up_silu_size_1 // 2, down_proj_size_2
                    )(i),
                )
            )
            ...
            def reduce_rms_rope_q_dep_notify(i, j, k, notify_idx):
                beg_idx = (
                    j // (self.NUM_ATTENTION_HEADS // self.NUM_KEY_VALUE_HEADS) * batch_size
                    + i * reduce_rms_rope_q_size_0
                )
                end_idx = j // (
                    self.NUM_ATTENTION_HEADS // self.NUM_KEY_VALUE_HEADS
                ) * batch_size + Tx.min((i + 1) * reduce_rms_rope_q_size_0, batch_size)
                ...
                R.call_tir_device(
                    ...
                    inverse_in_deps=rx.utils.Dependency(
                        ...
                        dep=lambda rank, x, inv_idx: (
                            Tx.if_then_else(
                                x < self.NUM_ATTENTION_HEADS, reduce_rms_rope_q_num_tiles_0, 0
                            ),
                            inv_idx,
                            x,
                            0,
                ...
                R.call_tir_device(
                    ...
                    inverse_in_deps=rx.utils.Dependency(
                        ...
                        dep=lambda rank, x, inv_idx: (
                            Tx.if_then_else(
                                tvm.tir.all(
                                    x >= self.NUM_ATTENTION_HEADS,
                                    x < self.NUM_ATTENTION_HEADS + self.NUM_KEY_VALUE_HEADS,
                                ),
                                reduce_rms_rope_append_k_num_tiles_0,
                                0,
                            ),
                            inv_idx,
                            x - self.NUM_ATTENTION_HEADS,
            ...
            kv_data_ = bb.emit(
                R.call_tir_device(
                    ...
                    in_deps=rx.utils.Dependency(
                        event=etensor_qkv_partial,
                        dep=lambda i, j, k, wait_idx: (
                            1,
                            -1,
                            j + self.NUM_ATTENTION_HEADS + self.NUM_KEY_VALUE_HEADS,
                        ),
                    ),
                    inverse_in_deps=rx.utils.Dependency(
                        event=etensor_qkv_partial,
            ...
            batch_attn_packed = bb.emit(
                R.call_tir_device(
                    ...
                    out_sinfo=[
                        rx.TensorStructInfo(
                            [batch_size, self.NUM_ATTENTION_HEADS, self.HEAD_DIM], "float16"
                        ),
                        rx.TensorStructInfo(
                            [
                                FuseBatchAttnTile.max_num_kv_splits
                                * self.NUM_KEY_VALUE_HEADS
                                * self.HEAD_DIM
                            ],
                            "float32",
                        ...
                        rx.TensorStructInfo(
                            [FuseBatchAttnTile.max_num_kv_splits * self.NUM_KEY_VALUE_HEADS],
                            "float32",
                        ),
                    ],
                    ...
                    out_deps=[
                        rx.utils.Dependency(
                            ...
                            dep=lambda i, j, k, notify_idx: (
                                Tx.if_then_else(
                                    attn_task_num > batch_size * self.NUM_KEY_VALUE_HEADS, 1, 0
                                ),
                                -1,
                                0,
                            ),
                        ...
                        rx.utils.Dependency(
                            ...
                            dep=lambda i, j, k, notify_idx: (
                                Tx.if_then_else(
                                    attn_task_num <= batch_size * self.NUM_KEY_VALUE_HEADS,
                                    self.SPLIT_O_PROJECT,
                                    0,
                                ),
                                -1,
    ...
    def get_mod(self, max_batch_size, profile_on=False):
        ...
        inner_blkm128 = self._qwen3_layer_inner(
            bb, max_batch_size, blk_m=128, profile_on=profile_on
        )
        ...
        with bb.function("megakernel", [x, residual, packed_info, packed_weights, workspace]):
            with bb.dataflow():
                ...
                if profile_on:
                    ...
                    output_shape = R.Tuple(
                        R.Tensor([batch_size, self.HIDDEN_SIZE], "float16"),
                ...
                def bind_branch_with_r_func(r_func, blk_m):
                    ...
                    bindings = [
                        rx.VarBinding(
                            ...
                            rx.Call(
                                r_func,
                                [x, residual, packed_info, packed_weights, workspace],
                                sinfo_args=output_sinfo,
                            ),
                        )
                    ]
                    bindings_blocks = [rx.BindingBlock(bindings)]
                    bindings_seq_expr = rx.SeqExpr(
                        bindings_blocks, bindings_blocks[-1].bindings[-1].var
                    )
                ...
                output_tuple = rx.If(
                    ...
                    bind_branch_with_if_else(
                        runtime_batch_size <= 64,
                        bind_branch_with_r_func(inner_blkm64, 64),
                        bind_branch_with_r_func(inner_blkm128, 128),
                    ),
                )
...
def get_packed_info(arg_dict, mk: MegaKernel):
    ...
    inverse_info = get_inverse_plan_info(
        batch_size,
        mk.NUM_KEY_VALUE_HEADS,
        arg_dict["q_indptr"],
