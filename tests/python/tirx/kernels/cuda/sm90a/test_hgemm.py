...
def test_hgemm_hopper_ws_cooperative():
    ...
    CLUSTER_M, CLUSTER_N, _CLUSTER_K = 2, 1, 1
    ...
    # replicate of cutlass3x_sm90_tensorop_s64x256x16gemm_f16_f16_f32_void_f32_128x256x64_2x1x1_0_ttt_align8_warpspecialized_cooperative_epi_tma  # noqa: E501
    def ceildiv(a, b):
        return (a + b - 1) // b
    ...
    @T.inline
    def tma_store(n_tile, C_smem: tvm.tirx.Buffer, C_map, m_glb_offset, n_glb_offset, tid):
    ...
    def manual(A_ptr: T.handle, B_ptr: T.handle, C_ptr: T.handle) -> None:
        A = T.match_buffer(A_ptr, (M, K), "float16", scope="global", layout=T.TileLayout(T.S[M, K]))
        B = T.match_buffer(B_ptr, (K, N), "float16", scope="global", layout=T.TileLayout(T.S[K, N]))
        C = T.match_buffer(C_ptr, (M, N), "float32", scope="global", layout=T.TileLayout(T.S[M, N]))
        ...
        C_map: T.let[T.handle("tensormap")] = T.tvm_stack_alloca("tensormap", 1)

        T.call_packed("runtime.cuTensorMapEncodeTiled", A_map, "float16", 2, A.data, K, M, f16_bytes * K, BLK_K, BLK_M, 1, 1, 0, swizzleA, 0, 0)  # noqa: E501
        T.call_packed("runtime.cuTensorMapEncodeTiled", B_map, "float16", 2, B.data, N, K, f16_bytes * N, 64, BLK_K // 2, 1, 1, 0, swizzleB, 0, 0) # multicast  # noqa: E501
        T.call_packed("runtime.cuTensorMapEncodeTiled", C_map, "float32", 2, C.data, N, M, f32_bytes * N, 32, BLK_M, 1, 1, 0, swizzleC, 0, 0)  # noqa: E501

        T.device_entry()
        cbx, cby = T.cta_id_in_cluster([CLUSTER_M, CLUSTER_N])
        bx, by = T.cta_id([CLUSTER_M, SM_COUNT // CLUSTER_M])
        wg_id = T.warpgroup_id([3])
        tid = T.thread_id([128 * 3])
        warp_id_in_wg = T.warp_id_in_wg([4])
        lane_id = T.lane_id([32])
        tid_in_wg = T.thread_id_in_wg([128])
                # tensor stroage
        A_smem = T.alloc_buffer([STAGES_MMA, BLK_M * BLK_K], "float16", scope="shared.dyn", align=128)  # noqa: E501
        B_smem = T.alloc_buffer([STAGES_MMA, BLK_K * BLK_N], "float16", scope="shared.dyn", align=128)  # noqa: E501
        ...
        full = T.alloc_buffer([STAGES_MMA], "uint64", scope="shared.dyn", layout=T.TileLayout(T.S[STAGES_MMA]), align=8)  # noqa: E501
        empty = T.alloc_buffer([STAGES_MMA], "uint64", scope="shared.dyn", layout=T.TileLayout(T.S[STAGES_MMA]), align=8)  # noqa: E501
                # work tile info
        ...
        consumer_release = PipelineState(1)
                # consumer signals
        is_signal_thread: T.int32
        dst_block_ID: T.int32
                # smem desc_A, desc_B
        desc_A: T.uint64
        desc_B: T.uint64
                # accumulators
                ...
                while (tile_scheduler.valid()):
                    if T.ptx.elect_sync():
                        ...
                        for i in range(k_tile_count):
                            ...
                            T.ptx.cp_async.bulk.tensor.g2c(2, A_smem.ptr_to([stage, 0]),cur_full, T.address_of(A_map), i * BLK_K, tile_scheduler.m_idx * BLK_M)  # noqa: E501
                            ...
                            for n_tile in range(4):
                                ...
                                T.ptx.cp_async.bulk.tensor.g2c(
                                    ...
                                    cur_full, T.address_of(B_map), tile_scheduler.n_idx * BLK_N + n_tile * 64, i * BLK_K + cbx * multicast_stride_b,  # noqa: E501
                                    cta_mask=0x3
                                )
            ...
            while (tile_scheduler.valid()):
                ...
                for k_iter in range(k_tile_count):
                    ...
                    for inner_k in range(BLK_K // WGMMA_K):
                        ...
                        T.ptx.wgmma.encode_matrix_descriptor(T.address_of(desc_A), A_smem.ptr_to([read_stage, A_offset]), 1, 64, swizzle=3)  # noqa: E501, F821
                        T.ptx.wgmma.encode_matrix_descriptor(T.address_of(desc_B), B_smem.ptr_to([read_stage, B_offset]), 512, 64, swizzle=3)  # noqa: E501, F821
                        T.ptx.wgmma.mma_async.ss(desc_A, desc_B, *[accum[i] for i in range(128)],  # noqa: F821
...
def test_hgemm_hopper_no_ws():
    ...
    def tma_load(tid, m_idx, n_idx, k_tile, A_smem: tvm.tirx.Buffer, B_smem: tvm.tirx.Buffer, A_map, B_map, bars: tvm.tirx.Buffer):  # noqa: E501
        if tid == 0:
            ...
            T.ptx.cp_async.bulk.tensor.g2c(2, A_smem.ptr_to([stage, 0]), bars.ptr_to([stage]), T.address_of(A_map), k_tile * BLK_K, m_idx * BLK_M)  # noqa: E501
            T.ptx.cp_async.bulk.tensor.g2c(2, B_smem.ptr_to([stage, 0]), bars.ptr_to([stage]), T.address_of(B_map), k_tile * BLK_K, n_idx * BLK_N)  # noqa: E501
    ...
    def mma_compute(wg_id, k_tile, A_smem: tvm.tirx.Buffer, B_smem: tvm.tirx.Buffer, accum, bars: tvm.tirx.Buffer, descA, descB):  # noqa: E501
        ...
        for inner_k in T.serial(BLK_K // WGMMA_K):
            ...
            T.ptx.wgmma.encode_matrix_descriptor(T.address_of(descA), A_smem.ptr_to([stage, A_offset]), 1, 64, swizzle=3)  # noqa: E501
            T.ptx.wgmma.encode_matrix_descriptor(T.address_of(descB), B_smem.ptr_to([stage, B_offset]), 1, 64, swizzle=3)  # noqa: E501
            T.ptx.wgmma.mma_async.ss(descA, descB, *get_accum_list(accum, 128),
                                     M=WGMMA_M, N=WGMMA_N, K=WGMMA_K, in_dtype="float16", out_dtype="float32", transA=False, transB=False, scaleA=1.0, scaleB=1.0, scaleD=True)  # noqa: E501
        T.ptx.wgmma.commit_group()
    ...
    def r2S(warp_id, lane_id, C_smem: tvm.tirx.Buffer, accum, accum_half, n_tile):
        ...
        for st_tile in T.serial(4):
            ...
            T.ptx.stmatrix(
                False, 4, ".b16",
                C_smem.ptr_to([n_tile % STAGES_EPI, row, col * 8]),
                accum_half.ptr_to([0]), accum_half.ptr_to([2]),
                accum_half.ptr_to([4]), accum_half.ptr_to([6]),
            )
    ...
    def s2G(warp_id, lane_id, C_smem: tvm.tirx.Buffer, C_map, m_idx, n_idx, n_tile):
        ...
        if warp_id == 0 and lane_id == 0:
            T.ptx.cp_async.bulk.tensor.s2g(2, C_smem.ptr_to([n_tile % STAGES_EPI, 0, 0]), T.address_of(C_map), n_idx * BLK_N + n_tile * 64, m_idx * BLK_M)  # noqa: E501
            T.ptx.cp_async.bulk.commit_group()
    ...
    def manual(A_ptr: T.handle, B_ptr: T.handle, C_ptr: T.handle) -> None:
        A = T.match_buffer(A_ptr, (M, K), "float16", scope="global", layout=T.TileLayout(T.S[M, K]))
        B = T.match_buffer(B_ptr, (N, K), "float16", scope="global", layout=T.TileLayout(T.S[N, K]))
        C = T.match_buffer(C_ptr, (M, N), "float16", scope="global", layout=T.TileLayout(T.S[M, N]))
        ...
        T.call_packed("runtime.cuTensorMapEncodeTiled", A_map, "float16", 2, A.data, K, M, f16_bytes * K, BLK_K, BLK_M, 1, 1, 0, swizzleA, 0, 0)  # noqa: E501
        T.call_packed("runtime.cuTensorMapEncodeTiled", B_map, "float16", 2, B.data, K, N, f16_bytes * K, BLK_K, BLK_N, 1, 1, 0, swizzleB, 0, 0)  # noqa: E501
        T.call_packed("runtime.cuTensorMapEncodeTiled", C_map, "float16", 2, C.data, N, M, f16_bytes * N, 64, BLK_M, 1, 1, 0, swizzleC, 0, 0)  # noqa: E501

        T.device_entry()
        bx = T.cta_id([SM_COUNT])
        wg_id = T.warpgroup_id([2])
        tid = T.thread_id([128 * 2])
        warp_id = T.warp_id([8])
        lane_id = T.lane_id([32])
                # tensor stroage
        A_smem = T.alloc_buffer([STAGES_TMA, BLK_M * BLK_K], "float16", scope="shared.dyn", align=128)  # noqa: E501
        B_smem = T.alloc_buffer([STAGES_TMA, BLK_K * BLK_N], "float16", scope="shared.dyn", align=128)  # noqa: E501
