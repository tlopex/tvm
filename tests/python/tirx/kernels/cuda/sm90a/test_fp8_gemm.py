...
def test_fp8_gemm_hopper_no_ws():
    ...
    def tma_load(tid, m_idx, n_idx, k_tile, A_smem: tvm.tirx.Buffer, B_smem: tvm.tirx.Buffer, A_map, B_map, bars: tvm.tirx.Buffer):  # noqa: E501
        if tid == 0:
            ...
            T.ptx.cp_async.bulk.tensor.g2c(2, A_smem.ptr_to([stage, 0]), bars.ptr_to([stage]), T.address_of(A_map), 0, 1, "", k_tile * BLK_K, m_idx * BLK_M)  # noqa: E501
            T.ptx.cp_async.bulk.tensor.g2c(2, B_smem.ptr_to([stage, 0]), bars.ptr_to([stage]), T.address_of(B_map), 0, 1, "", k_tile * BLK_K, n_idx * BLK_N)  # noqa: E501
    ...
    def mma_compute(wg_id, k_tile, A_smem: tvm.tirx.Buffer, B_smem: tvm.tirx.Buffer, accum, bars: tvm.tirx.Buffer, descA, descB):  # noqa: E501
        ...
        for inner_k in T.serial(BLK_K // WGMMA_K):
            ...
            T.ptx.wgmma.mma_async.ss(descA, descB, *get_accum_list(accum, 128), M=WGMMA_M, N=WGMMA_N, K=WGMMA_K, in_dtype="float8_e4m3fn", out_dtype="float32", transA=False, transB=False, scaleA=1.0, scaleB=1.0, scaleD=True)  # noqa: E501
    ...
    def manual(A_ptr: T.handle, B_ptr: T.handle, C_ptr: T.handle) -> None:
        A = T.match_buffer(A_ptr, (M, K), "float8_e4m3fn", scope="global", layout=T.TileLayout(T.S[M, K]))  # noqa: E501
        B = T.match_buffer(B_ptr, (N, K), "float8_e4m3fn", scope="global", layout=T.TileLayout(T.S[N, K]))  # noqa: E501
        ...
        T.call_packed("runtime.cuTensorMapEncodeTiled", A_map, "float8_e4m3fn", 2, A.data, K, M, f8_bytes * K, BLK_K, BLK_M, 1, 1, 0, swizzleA, 0, 0)  # noqa: E501
        T.call_packed("runtime.cuTensorMapEncodeTiled", B_map, "float8_e4m3fn", 2, B.data, K, N, f8_bytes * K, BLK_K, BLK_N, 1, 1, 0, swizzleB, 0, 0)  # noqa: E501
        ...
        wg_id = T.warpgroup_id([2])
                # tensor stroage
        A_smem = T.alloc_buffer([STAGES_TMA, BLK_M * BLK_K], "float8_e4m3fn", scope="shared.dyn", align=128)  # noqa: E501
        B_smem = T.alloc_buffer([STAGES_TMA, BLK_K * BLK_N], "float8_e4m3fn", scope="shared.dyn", align=128)  # noqa: E501
    ...
    def cublas_gemm():
        ...
        def func():
            return gemm_func(A_tvm, B_tvm, workspace, scale, C_tvm)
