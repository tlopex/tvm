...
def test_partial_reduction():
    ...
    def int_var(name: str, scope="local"):
        return T.alloc_buffer([1], "int32", align=4, scope=scope)
    ...
    @T.meta_class
    class MPMCQueue:
        def __init__(self, capacity: int, task_types: T.Buffer, task_idxs: T.Buffer, head: T.Buffer, tail: T.Buffer, num_tot_tasks: int):  # noqa: E501
        ...
        def enqueue(self, task_type: int, *task_idx: int):
            self.tail_r[0] = T.cuda.atomic_add(self.tail.access_ptr("rw", offset=self.tail.elem_offset_of([T.int32(0)])), 1)  # noqa: E501
        ...
        def dequeue(self, fetched_task_type: T.Buffer, fetched_task_idx: T.Buffer):
            self.head_r[0] = T.cuda.atomic_add(self.head.access_ptr("rw", offset=self.head.elem_offset_of([T.int32(0)])), 1)  # noqa: E501
            if self.head_r[0] < self.num_tot_tasks:
              self.masked_pos[0] = self.head_r[0] & self.mask
              T.ptx.ld_global_acquire(fetched_task_type[0], self.task_types.access_ptr("r", offset=self.task_types.elem_offset_of([self.masked_pos[0]])))  # noqa: E501
    ...
    @T.meta_class
    class Semaphore:
        def __init__(self, cnt: int, buffer: T.Buffer, queue: MPMCQueue, tid):
            self.cnt = cnt
            self.sem = buffer
        ...
        def semaphore_notify(self, *coord):
            ...
            if self.tid == 0:
                ...
                self.state[0] = T.cuda.atomic_add(self.sem.access_ptr("rw", offset=self.sem.elem_offset_of(coord)), 1) + 1  # noqa: E501
    ...
    @T.prim_func
    def partial_reduction_fused(A: T.handle, B: T.handle, C: T.handle, semaphore: T.handle, task_types: T.handle, task_idxs: T.handle, head: T.handle, tail: T.handle):  # noqa: E501
        A_ptr = T.match_buffer(A, (M, N), "float32")
        B_ptr = T.match_buffer(B, (M, NUM_BLOCK_N), "float32")
        C_ptr = T.match_buffer(C, (M, 1), "float32")
