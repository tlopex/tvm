    ...
    def expected():
        ...
        with T.kernel():
            ...
            C_sbuf = T.alloc_buffer((512, 1024), scope="trn.sbuf",
                                    layout=T.TileLayout(([128, 4096], [1@P, 1@F])))
            Tx.exp(C_sbuf[0:512, 0:1024], A_sbuf[0:512, 0:1024], T.float32(1.0), T.float32(2.0), workspace={"const_bias": const_bias})
    # fmt: on
    with target:
    ...
    def expected():
        ...
        with T.kernel():
            ...
            for i, k in T.grid(2, 2):
                Tx.gemm(C_sbuf[256 * i:256 * i + 256, 0:256], A_sbuf[256 * i:256 * i + 256, 512 * k:512 * k + 512], B_sbuf[512 * k:512 * k + 512, 0:256], C_sbuf[256 * i:256 * i + 256, 0:256], False, False, T.float32(1.0), T.float32(0.0), workspace={"acc_psum": acc_psum})
    # fmt: on
    with target:
    ...
    def expected():
        ...
        with T.kernel():
            ...
            C_sbuf = T.alloc_buffer((512,), scope="trn.sbuf",
                                    layout=T.TileLayout(([128, 4], [(1, "P"), (1, "F")])))
            Tx.binary_reduce(B_sbuf[0:512, 0:1024, 0:4], C_sbuf[0:512], A_sbuf[0:512, 0:1024, 0:4], T.float32(1.0), "add", "sum", [1, 2], workspace={"partial_reduce": partial_reduce})
    # fmt: on
    with target:
...
def test_workspace_reuse():
    ...
    def expected():
        ...
        with T.kernel():
            ...
            C_sbuf = T.alloc_buffer((512, 1024), scope="trn.sbuf",
                                    layout=T.TileLayout(([128, 4096], [(1, "P"), (1, "F")])))
            Tx.exp(C_sbuf[0:512, 0:1024], A_sbuf[0:512, 0:1024], T.float32(0.0), T.float32(2.0), workspace={"const_bias": const_bias}, max_inst_size=1024)
