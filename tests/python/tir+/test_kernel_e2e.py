...
def test_gemm_ampere():
    ...
    @T.prim_func(tirp=True)
    def op(A_ptr: T.handle, B_ptr: T.handle, C_ptr: T.handle) -> None:
        ...
        with T.kernel():
            ...
            with T.cta():
                ...
                for k in T.serial(T.ceildiv(K, BLK_K)):
                    ...
                    T.static_assert(N % BLK_N == 0, f"N={N}, BLK_N={BLK_N}")
                    T.static_assert(K % BLK_K == 0, f"K={K}, BLK_K={BLK_K}")
                    Tp.copy(A_smem[:, :], A[bx * BLK_M: bx * BLK_M + BLK_M, k * BLK_K: k * BLK_K + BLK_K])
                    Tp.copy(B_smem[:, :], B[by * BLK_N: by * BLK_N + BLK_N, k * BLK_K: k * BLK_K + BLK_K])
    ...
    @T.prim_func(tirp=True)
    def pipeline(A_ptr: T.handle, B_ptr: T.handle, C_ptr: T.handle) -> None:
        A = T.match_buffer(A_ptr, (M, K), "float16", scope="global", layout=T.TileLayout.from_nested_tuple((M, K)))
        B = T.match_buffer(B_ptr, (N, K), "float16", scope="global", layout=T.TileLayout.from_nested_tuple((N, K)))
        C = T.match_buffer(C_ptr, (M, N), "float32", scope="global", layout=T.TileLayout.from_nested_tuple((M, N)))
    ...
    with target:
        C_tvm_raw = tvm_gemm(raw, "raw")
        C_tvm_op = tvm_gemm(op, "op")
        C_tvm_pipeline = tvm_gemm(pipeline, "pipeline")
        C_cublas = cublas_gemm()

    tvm.testing.assert_allclose(C_tvm_raw.asnumpy(), C_cublas.asnumpy(), rtol=1e-3, atol=1e-3)
    tvm.testing.assert_allclose(C_tvm_op.asnumpy(), C_cublas.asnumpy(), rtol=1e-3, atol=1e-3)
    tvm.testing.assert_allclose(C_tvm_pipeline.asnumpy(), C_cublas.asnumpy(), rtol=1e-3, atol=1e-3)
    print("test passed")
