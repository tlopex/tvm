        ...
        with T.kernel():
            ...
            with T.thread():
                ...
                with T.warp():
                    ...
                    if op_type == "sum":
                        Tp.sum(red_view, acc_view, thread_reduce=shuffle)
                    elif op_type == "max":
                        Tp.max(red_view, acc_view, thread_reduce=shuffle)
                    # perform an additional shuffle step if not shuffled above
                    if not shuffle:
                        if op_type == "sum":
                            Tp.sum(red_view, red_view, thread_reduce=True)
                        elif op_type == "max":
                            Tp.max(red_view, red_view, thread_reduce=True)
