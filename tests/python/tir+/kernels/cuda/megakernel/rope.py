                ...
                while self.idx[0] < self.m_tile * self.h_tile and m_idx * self.m_tile + self.idx[0] // self.h_tile < self.batch_size:
                    ...
                    if batch_idx < self.batch_size and head_idx < self.qo_heads + self.kv_heads:
                        ...
                        for kv in T.unroll(self.vec_size // 2):
                            half22float2(T.address_of(self.qk_vec32[kv * 2]), T.address_of(self.qk_vec[kv * 2]))
                        ...
                        for kv in T.serial(self.vec_size):
                            self.qk_vec32_other[kv] = T.tvm_warp_shuffle_xor(self.mask[0], self.qk_vec32[kv], self.bdx // 2, 32, 32)

                        # compute rope
                        if stx < half_dim:
                            for kv in T.unroll(self.vec_size):
                                self.qk_vec32[kv] = self.qk_vec32[kv] * self.cos[kv] - self.qk_vec32_other[kv] * self.sin[kv]
                        else:
                            for kv in T.unroll(self.vec_size):
                                self.qk_vec32[kv] = self.qk_vec32[kv] * self.cos[kv] + self.qk_vec32_other[kv] * self.sin[kv]
                        ...
                        # store qk
                        for kv in T.unroll(self.vec_size // 2):
                            float22half2(T.address_of(self.qk_vec[kv * 2]), T.address_of(self.qk_vec32[kv * 2]))
                        for kv in T.unroll(self.vec_size):
