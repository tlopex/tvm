...
class TileScheduler:
    ...
    def update_current_m_n_idx(self, linear_idx):
        ...
        if linear_idx < group_rows * self.n_clusters and group_rows > 0:
            self.m_idx[0] = linear_idx // group_repeat * GROUP_SIZE + (linear_idx % GROUP_SIZE)
            self.n_idx[0] = linear_idx % group_repeat // GROUP_SIZE
        elif final_rows > 0:
            remainder_idx = linear_idx - group_rows * self.n_clusters
            self.m_idx[0] = group_rows + remainder_idx % final_rows
            self.n_idx[0] = remainder_idx // final_rows
    ...
    @T.macro
    def next_tile(self):
        self.linear_idx[0] = self.linear_idx[0] + SM_COUNT // 2
        self.update_current_m_n_idx(self.linear_idx[0])

    def valid(self):
        return self.linear_idx[0] < self.m_clusters * self.n_clusters
...
class Pipeline:
    def __init__(
        ...
        self.idx = int_var()
        self.p2c_phase = int_var()
        self.c2p_phase = int_var()
        self.p_single_cta = p_single_cta
        self.c_single_cta = c_single_cta
        IRBuilder.current().name("pipeline_idx", self.idx)
        IRBuilder.current().name("pipeline_p2c_phase", self.p2c_phase)
        IRBuilder.current().name("pipeline_c2p_phase", self.c2p_phase)
    ...
    @T.macro
    def advance(self):
        self.idx[0] = (self.idx[0] + 1) % self.pipeline_depth
        if self.idx[0] == 0:
            self.p2c_phase[0] = self.p2c_phase[0] ^ 1
            self.c2p_phase[0] = self.c2p_phase[0] ^ 1
        ...
        for cbx in T.thread_binding(CLUSTER_M, "clusterCtaIdx.x"):
            if not self.p_single_cta or cbx == 0:
                T.ptx.mbarrier.try_wait(
                    self.mbar_c2p.access_ptr(
                        "rw", offset=self.mbar_c2p.offset_of_p([self.idx[0], pipeline_idx])
                    ),
                    self.c2p_phase[0],
                )
    ...
    def consumer_wait(self, pipeline_idx):
        for cbx in T.thread_binding(CLUSTER_M, "clusterCtaIdx.x"):
            if not self.c_single_cta or cbx == 0:
                T.ptx.mbarrier.try_wait(
                    self.mbar_p2c.access_ptr(
                        "rw", offset=self.mbar_p2c.offset_of_p([self.idx[0], pipeline_idx])
                    ),
                    self.p2c_phase[0],
                )
    ...
    def test_mma_ss_tma_2sm_persistent(A: T.Buffer((M, K), a_type, layout="default"), B: T.Buffer((N, K), b_type, layout="default"), C: T.Buffer((M, N), d_type, layout="default")):
        ...
        with T.kernel():
            ...
            with T.cta():
                ...
                T.ptx.tcgen05.encode_instr_descriptor(descI.data, "float32", a_type, b_type, MMA_M, MMA_N, MMA_K, trans_a=False, trans_b=False, n_cta_groups=cta_group)
                    ...
                    with T.warpgroup()[NUM_CONSUMER:NUM_CONSUMER + 1]:
                        ...
                        if warp_id == 3:
                            ...
                            while tile_scheduler.valid():
                                m_idx = T.meta_var(tile_scheduler.m_idx[0]) # represent cluster task id
                                n_idx = T.meta_var(tile_scheduler.n_idx[0])
                                for ko in range(K // BLK_K):
                                    # GMEM -> SMEM
                                    if lane_id == 0:
                                        tma2mma_pipe.producer_wait(0)
                                        if not first_iter[0] and ko == PIPE_DEPTH - 1:
                                        ...
                                        T.ptx.cp_async.bulk.tensor.g2c(2, 
                                                                        A_smem.access_ptr("rw", offset=A_smem.offset_of_p([tma2mma_pipe.idx[0], 0, 0, 0])),
                                                                        tma_finished.access_ptr("rw", offset=tma_finished.offset_of_p([tma2mma_pipe.idx[0]])),
                                                                        A_tensor_map,
                                                                        ko * BLK_K,
                                                                        (m_idx * 4 + cbx) * BLK_M,
                                                                        cta_group=2)
                                        T.ptx.cp_async.bulk.tensor.g2c(2, 
                                                                        A_smem.access_ptr("rw", offset=A_smem.offset_of_p([tma2mma_pipe.idx[0], 1, 0, 0])),
                                                                        tma_finished.access_ptr("rw", offset=tma_finished.offset_of_p([tma2mma_pipe.idx[0]])),
                                                                        A_tensor_map,
                                                                        ko * BLK_K, 
                                                                        (m_idx * 4 + 2 + cbx) * BLK_M,
                                                                        cta_group=2)
                                        T.ptx.cp_async.bulk.tensor.g2c(2, 
                                                                        B_smem.access_ptr("rw", offset=B_smem.offset_of_p([tma2mma_pipe.idx[0], 0, 0])),
                                                                        tma_finished.access_ptr("rw", offset=tma_finished.offset_of_p([tma2mma_pipe.idx[0]])),
                                                                        B_tensor_map, 
                                                                        ko * BLK_K, 
                                        ...
                                        if cbx == 0:
                                            ...
                                            T.ptx.mbarrier.arrive.expect_tx(tma_finished.access_ptr("rw", offset=tma_finished.offset_of_p([tma2mma_pipe.idx[0]])), (BLK_K * BLK_M * 2 * 2 + BLK_K * BLK_N) * 2)
                                    ...
                                    if lane_id == 0 and cbx == 0:
                                        ...
                                        for ko in T.serial(0, K // BLK_K):
                                            ...
                                            for ki in range(BLK_K // MMA_K):
                                                T.ptx.tcgen05.encode_matrix_descriptor(descA.data, A_smem.access_ptr("r", offset=A_smem.offset_of_p([tma2mma_pipe.idx[0], warp_id, 0, ki * MMA_K])), ldo=ldo, sdo=sdo, swizzle=SWIZZLE)
                                                T.ptx.tcgen05.encode_matrix_descriptor(descB.data, B_smem.access_ptr("r", offset=B_smem.offset_of_p([tma2mma_pipe.idx[0], 0, ki * MMA_K])), ldo=ldo, sdo=sdo, swizzle=SWIZZLE)
                                                if ki == 0 and ko == 0:
                                                    T.ptx.tcgen05.mma("float32", a_type, b_type, tmem_addr[0] + warp_id * MMA_N, descA[0], descB[0], descI[0], use_a_tmem=False, cta_group=cta_group, enable_input_d=False)
                                                else:
                                                    T.ptx.tcgen05.mma("float32", a_type, b_type, tmem_addr[0] + warp_id * MMA_N, descA[0], descB[0], descI[0], use_a_tmem=False, cta_group=cta_group, enable_input_d=True)
                    ...
                    with T.warpgroup()[0:NUM_CONSUMER]:
                        ...
                        while tile_scheduler.valid():
                            ...
                            for i in range(BLK_N // TMEM_LD_SIZE):
                                ...
                                T.ptx.tcgen05.ld(tmem_addr[0] + wg_id * MMA_N, warp_id * 32, i * TMEM_LD_SIZE, "32x32b", TMEM_LD_SIZE, False, *regs)
