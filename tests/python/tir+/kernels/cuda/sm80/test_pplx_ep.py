...
def test_dispatch_combine(world_size):
    np.random.seed(42)
    ...
    def warp_dispatch_exp(
        group_idx, warp_id, token_idx, smem_buf, send_experts, buf_send, buf_recv, buf_actual_wait
    ):
            ...
            if exp_idx < K:
                ...
                T.nvshmem.putmem_signal_nbi.warp(
                    dst=buf_recv.access_ptr(
                        "w",
                        offset=buf_recv.offset_of_p([dst_local_expert, group_idx, dst_index, 0]),
                    ),
            ...
            for k in T.serial(T.min(N_RECV_GROUPS_PER_CTA, N_RECV_GROUPS - group_offset_cta)):
                ...
                for t in T.serial(T.ceildiv(num_recv_tokens, n_threads)):
                    ...
                    if token_idx < num_recv_tokens:
                        ...
                        buf_meta_offset[meta_idx] = T.cast(smem_expert_st[k] + token_idx, "uint32")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        dispatch_mod = tvm.compile(dispatch_kernel, target=target, tir_pipeline="tirp")
        dispatch_mod.export_library(dispatch_path)
        combine_path = tmpdir + "/combine.so"
        combine_mod = tvm.compile(combine_kernel, target=target, tir_pipeline="tirp")
    ...
    mask = np.any(mask, axis=2).transpose(2, 3, 0, 1)  # (world_size, N_LOCAL_EXPERTS, N_GROUP, M)
...
if __name__ == "__main__":
    test_dispatch_combine(world_size=8)
