        ...
        with T.kernel():
            ...
            with T.thread():
                ...
                def compute_tile(m_idx, n_idx):     
                    ...
                    Tp.copy(rmap[:], sorted_token_ids[m_idx * BLK_M : (m_idx + 1) * BLK_M])
                    ...
                    def store_C_to_smem():
                        for mma_m in range(NUM_MMA_M):
                            for mma_n in range(NUM_MMA_N):
                                ...
                                Tp.cast(s_frag_f16[:], s_frag[mma_m, mma_n, :])
    ...
    def tir():
        ...
        for tir_config in config_space:
            ...
            with target:
                ...
                mod1 = tvm.compile(mod1, target=target, tir_pipeline="tirp")

                mod2 = tvm.IRModule({"main": grp_gemm2})
                mod2 = tvm.compile(mod2, target=target, tir_pipeline="tirp")
