...
def test_relax_deep_variable_dependency_chain():
    ...
    O(2^depth) and hanging tirx.LowerTIRxCleanup indefinitely. The relaxation is
    now memoized per variable, so this completes in linear time.
    """
    ck = IntSetChecker()
