...
TEST(Tuple, DefaultConstructor) {
  ...
  Tuple<String, Integer> t1(String("Hello"), Integer(42));
  ASSERT_EQ(t1.GetElement<0>(), "Hello");
  ASSERT_EQ(t1.GetElement<1>(), 42);

  String s = String("TVM");
  Tuple<String> t2(s);
  ASSERT_EQ(t2.GetElement<0>(), "TVM");
}

TEST(Tuple, InitializerListConstructor) {
  // construct from initializer list
  Tuple<Integer, String> t = {1, "abc"};
  ASSERT_EQ(t.GetElement<0>(), 1);
  ASSERT_EQ(t.GetElement<1>(), "abc");
}

TEST(Tuple, MoveConstructor) {
  // // move constructor
  Tuple<String, Integer> t1(String("Hello"), Integer(42));
  Tuple<String, Integer> t2(std::move(t1));
  ASSERT_EQ(t2.GetElement<0>(), "Hello");
  ASSERT_EQ(t2.GetElement<1>(), 42);
  ASSERT_FALSE(t1.defined());
}

TEST(Tuple, CopyConstructor) {
  // // copy constructor
  Tuple<String, Integer> t2(String("Hello"), Integer(42));
  ASSERT_TRUE(t2.unique());
  Tuple<String, Integer> t3(t2);
  ASSERT_EQ(t3.GetElement<0>(), "Hello");
  ASSERT_EQ(t3.GetElement<1>(), 42);
  ASSERT_TRUE(t3.same_as(t2));
  ASSERT_TRUE(!t2.unique());
  ASSERT_TRUE(!t3.unique());
  ASSERT_EQ(t2.use_count(), 2);
  ASSERT_EQ(t3.use_count(), 2);
}

TEST(Tuple, ElementGet) {
  Tuple<Array<Integer>, Variant<Integer, String>> t(Array<Integer>{1, 2, 3}, Integer(42));
  ASSERT_EQ(t.GetElement<0>()[0], 1);
  ASSERT_EQ(t.GetElement<0>()[1], 2);
  ASSERT_EQ(t.GetElement<0>()[2], 3);
  ASSERT_EQ(t.GetElement<1>().as<Integer>(), 42);
}

TEST(Tuple, Assignment) {
  // move assignment
  Tuple<Integer, String> orig(1, "abc");
  Tuple<Integer, String> target = std::move(orig);
  ASSERT_EQ(target.GetElement<0>(), 1);
  ASSERT_EQ(target.GetElement<1>(), "abc");
  ASSERT_TRUE(!orig.defined());
  ASSERT_TRUE(target.unique());

  // copy assignment
  Tuple<Integer, String> orig1(2, "TVM");
  ASSERT_TRUE(orig1.unique());
  Tuple<Integer, String> target1 = orig1;
  ASSERT_EQ(orig1.GetElement<0>(), 2);
  ASSERT_EQ(target1.GetElement<0>(), 2);
  ASSERT_TRUE(target1.same_as(orig1));
  ASSERT_TRUE(!orig1.unique());
  ASSERT_TRUE(!target1.unique());
  ASSERT_EQ(orig1.use_count(), 2);
  ASSERT_EQ(target1.use_count(), 2);
}

TEST(Tuple, CheckEquality) {
  // copy
  Tuple<Array<Integer>, Integer, String> orig1(Array<Integer>{1, 2, 3}, 42, "TVM");
  Tuple<Array<Integer>, Integer, String> target1 = orig1;
  ASSERT_TRUE(orig1.same_as(target1));
  ASSERT_TRUE(orig1 == target1);
  ASSERT_FALSE(orig1 != target1);
  ASSERT_TRUE(ObjectEqual()(orig1, target1));

  // string-based check
  Tuple<Integer, Integer> orig2(41, 42);
  Tuple<Tuple<Integer, Integer>, String> target3(orig2, String("TVM"));
  Tuple<Tuple<Integer, Integer>, String> target4(orig2, String("TVM"));
  ASSERT_TRUE(!target3.same_as(target4));
  ASSERT_TRUE(target3 == target4);
  ASSERT_FALSE(target3 != target4);
  ASSERT_TRUE(ObjectEqual()(target3, target4));
}

TEST(Tuple, CheckInequality) {
  // diff type
  Tuple<Integer, Integer> t(1, 42);
  Tuple<Array<Integer>, Integer> t1(Array<Integer>{1, 2, 3}, 42);
  ASSERT_TRUE(!ObjectEqual()(t, t1));

  // pointer-based check
  Tuple<Integer> t2(42);
  Tuple<Integer> t3(42);
  ASSERT_TRUE(!t2.same_as(t3));
  ASSERT_TRUE(t2 != t3);
  ASSERT_FALSE(t2 == t3);
  ASSERT_TRUE(!ObjectEqual()(t2, t3));
}

TEST(Tuple, Concat) {
  Tuple<String, Integer> t1("hello", 1);
  Tuple<Integer> t2(2);
  Tuple<String, Integer, Integer> t3 = Concat(t1, t2);
  ASSERT_EQ(t3.GetElement<0>(), "hello");
  ASSERT_EQ(t3.GetElement<1>(), 1);
  ASSERT_EQ(t3.GetElement<2>(), 2);
  ASSERT_NE(t3.get(), t1.get());
  ASSERT_NE(t3.get(), t2.get());
}
